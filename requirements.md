...
## 7. Приоритеты MVP и критерии готовности

| Раздел | Приоритет | Критерии готовности |
| --- | --- | --- |
| Dashboard | P2 (желательно к демо) | Карточка с агрегированными метриками дня/недели (сумма закупок, сумма продаж, ожидаемый доход), индикатор лимитов Cloudflare обновляется хотя бы раз в сутки. |
| Hand‑Trade | P1 (обязательно) | Форма ручного добавления товара (игра, тип ресурса, цена, описание, площадка), сохранение в базе, статус «черновик/готов к продаже», просмотр списка своих товаров. |
| Склад | P1 (обязательно) | Просмотр списка выкупленных товаров, редактирование цены/описания, перевод товара в «Витрину». Минимальная фильтрация по игре и статусу. |
| Витрина | P1 (обязательно) | Таблица активных лотов с ценой закупки/продажи, маржей и ссылками на Taobao/биржу, смена статуса «новый→опубликован→продан». |
| Настройки | P0 (критично) | Ввод/редактирование токена RapidAPI, параметры наценки и курса валют, валидация и сохранение в хранилище. |

## 8. Минимальный набор API для среза MVP

* **Поиск товара:** `GET /api/taobao/search` — проксирует запрос к Just One API, принимает параметры игры/категории/цены и возвращает краткие карточки (название, цена, ссылки, изображения).
* **Добавление товара вручную:** `POST /api/items` — сохраняет товар из Hand‑Trade (игра, ресурс, цена закупки/продажи, описание, площадка, статус).
* **Статусы и переходы:** `POST /api/items/{id}/status` — смена статуса между «черновик», «склад», «витрина», «опубликован», «продан»; фиксирует дату и пользователя операции.
* **Получение списков:** `GET /api/items` — список товаров с фильтрами по разделам (Hand‑Trade/Склад/Витрина/Продано) и статусам.
* **Настройки токена:** `GET/POST /api/settings/token` — чтение и сохранение RapidAPI‑токена, валидация формата, проверка тестовым запросом к Just One API.

## 9. План тестирования MVP

### 9.1 Ручные сценарии

1. **Hand‑Trade:** создать товар с обязательными полями → убедиться, что он появляется в списке и сохраняется после обновления страницы.
2. **Склад ↔ Витрина:** перевести товар в статус «склад», отредактировать цену/описание → отправить в «Витрину» → статус меняется, данные сохраняются.
3. **Продажа:** сменить статус «опубликован» на «продан» → фиксируется дата продажи, маржа пересчитывается в таблице.
4. **Поиск Taobao:** выполнить поиск по игре с заданным диапазоном цены → вернуть не менее одной карточки с корректными ссылками и ценой.
5. **Настройки токена:** ввести валидный RapidAPI‑токен → сохранить → выполнить тестовый запрос через UI (проверка 200 OK); ввести невалидный токен → получить ошибку валидации.
6. **Доступы:** под ролями Admin и Trader проверить ограничение видимости разделов (Trader видит только Hand‑Trade и свои товары).

### 9.2 Smoke‑чек‑лист (каждый билд)

* UI открывается без ошибок в консоли браузера; навигация между разделами работает.
* `GET /api/taobao/search` возвращает 200 и непустой список по тестовому запросу.
* `POST /api/items` создаёт запись, которая отображается в Hand‑Trade без перезагрузки страницы.
* `POST /api/items/{id}/status` меняет статус и отражается в списках Склад/Витрина.
* Сохранение RapidAPI‑токена через `POST /api/settings/token` и успешный тестовый запрос подтверждают корректность учётных данных.
