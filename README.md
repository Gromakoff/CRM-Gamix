...
Доступ к статике админки защищён HTTP Basic Auth. По умолчанию используются `gth-admin` / `gth-protect-9824`; при деплое можно переопределить логин и пароль переменными `ADMIN_GATE_USER` и `ADMIN_GATE_PASS`.
...
4. Открыть https://game-trade-hub-worker.gromakoff.workers.dev/ — доступ защищён базовой авторизацией (по умолчанию `gth-admin` / `gth-protect-9824`). Там есть ссылки на регистрацию по email и через Telegram. При необходимости переопределите базу через `?apiBase=`.
