...
Доступ к статике админки защищён встроенной страницей с вводом пароля (в стиле самой админки). По умолчанию пароль `gth-protect-9824`, его можно переопределить переменной `ADMIN_GATE_PASS`. Вводится только пароль — без логина и без системного окна браузера.
...
Админка поддерживает обновление статуса с комментарием, просмотр истории, удаление товара и получение сводки/трендов по API. Регистрация вынесена на отдельную страницу `/register.html` (email/пароль), а вход через Telegram открывается одной ссылкой `/telegram-login`. После успешной регистрации пользователь сразу перенаправляется в основную админку.
...
4. Открыть https://gmhub.gromakoff.workers.dev/ — доступ закрыт страницей ввода пароля (по умолчанию `gth-protect-9824`, можно поменять через `ADMIN_GATE_PASS`). После ввода сразу откроются ссылки на регистрацию по email и через Telegram. При необходимости переопределите базу через `?apiBase=`.
